...
def get_feedback_api_rec(pair, query_matrix, query_idf_vector, w2v, idf):
    line = 0
    feeds = []
    for row in pair:
        if line > 0:
...
def get_feedback_inf_rec(test, pair, rec_api_test, w2v, idf):
...
def get_feedback_inf(test, question, answer, rec_api_test, w2v, idf):
    feedback_inf = []
    # print(111111111, len(test), type(test), test)
    if isinstance(test, str):
        test = [test]
