...
for train_idx, test_idx in kf.split(queries):
    ...
    for round in range(1):
        ...
        choose_idx = random.sample(list(train_idx), num_choose)
        print(choose_idx)
        print(len(train_idx), len(test_idx), len(choose_idx))
        print('---------------------')

        # get the training, testing, feedback data according to their idx
        split_data.idx_to_data(train_idx, choose_idx, test_idx)
    ...
    #
    # fw = open(path.join(path1, './biker_metric_shareFR.csv'), 'a+', newline='')
